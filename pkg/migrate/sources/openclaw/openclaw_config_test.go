...
func TestConvertToOperator(t *testing.T) {
	...
	picoCfg, warnings, err := cfg.ConvertToOperator("")
	if err != nil {
	...
	if picoCfg.Agents.Defaults.Workspace != "~/.operator/workspace" {
		t.Errorf("expected workspace '~/.operator/workspace', got '%s'", picoCfg.Agents.Defaults.Workspace)
	}
...
func TestConvertToOperatorWithQQAndDingTalk(t *testing.T) {
	...
	picoCfg, _, err := cfg.ConvertToOperator("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}
